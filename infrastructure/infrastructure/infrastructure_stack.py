    ...
    def __init__(
        ...
        **kwargs,
        ...
        dynamodb_policy = iam.Policy(
            ...
            statements=[
                iam.PolicyStatement(
                    ...
                    resources=[
                        f"arn:aws:dynamodb:{self.region}:{self.account}:table/next-auth",
                        f"arn:aws:dynamodb:{self.region}:{self.account}:table/next-auth/index/GSI1",
                    ],
                )
            ],
        )
        ...
        dynamodb_assignment_policy = iam.Policy(
            ...
            statements=[
                iam.PolicyStatement(
                    ...
                    resources=[
                        f"arn:aws:dynamodb:{self.region}:{self.account}:table/{env_name}-AssignmentsTable",
                    ],
                )
            ],
        )
        ...
        llm_generation_resource.add_method(
            "POST",
            apigateway.LambdaIntegration(llm_generation_function),
            api_key_required=True,  # the endpoint is only reachable with a valid x-api-key header
        )
        ...
        # API key + usage plan. The key value is generated by AWS, never stored in the repo:
        # read it once from the console (or `aws apigateway get-api-key --include-value`)
        # and set it as the LLM_API_KEY environment variable on the web app.
        orchestration_api_key = orchestration_api.add_api_key(
            f"{env_name}-OrchestrationApiKey",
            api_key_name=f"{env_name}-OrchestrationApiKey",
        )

        orchestration_usage_plan = orchestration_api.add_usage_plan(
            f"{env_name}-OrchestrationUsagePlan",
            name=f"{env_name}-OrchestrationUsagePlan",
            throttle=apigateway.ThrottleSettings(rate_limit=10, burst_limit=20),
        )
        orchestration_usage_plan.add_api_stage(stage=orchestration_api.deployment_stage)
        orchestration_usage_plan.add_api_key(orchestration_api_key)

        CfnOutput(
            self,
            "OrchestrationApiEndpoint",
            value=orchestration_api.url_for_path("/llm_generation"),
            description="Set this as LLM_API_ENDPOINT on the web app",
        )
        CfnOutput(
            self,
            "OrchestrationApiKeyId",
            value=orchestration_api_key.key_id,

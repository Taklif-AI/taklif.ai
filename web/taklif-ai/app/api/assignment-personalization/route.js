...
export async function POST(req) {
  ...
  try {
    ...
    const validated_interest = assignmentSchema.safeParse({
      student_interest: body.student_interest,
    });
    if (!validated_interest.success) {
      return new Response(
        JSON.stringify({ error: validated_interest.error.message }),
        { status: 400 },
      );
    }
    ...
    if (body.is_pdf) {
      ...
      if (
        !body.general_assignment &&
        !(body.general_assignment.trim().length > 0)
      ) {
        return new Response(
          JSON.stringify({ error: "General assignment can not be empty" }),
          { status: 400 },
        );
      }
    }
    ...
    const dataToApi = {
      ...
      params: {
        ...
        is_pdf: body.is_pdf,
        general_assignment: body.is_pdf
          ? pdf_base64_data
          : body.general_assignment,
        run_id: body.run_id,
    ...
    const res = await fetch(
      "https://ne4754rnb8.execute-api.eu-north-1.amazonaws.com/Development/llm_generation",
      {
        method: "POST",
    ...
    return new Response(
      JSON.stringify({ success: true, customized_assignment: data }),
      { status: 200 },
    );
  } catch (error) {
    console.log(error);
